...
export interface ProjectCardSmallProps {
  ...
  href?: string
  linkText?: string
  color?: 'emerald' | 'blue' | 'green' | 'purple' | 'red' | 'orange' | 'yellow' | 'slate'
}
    ...
    >
      ...
      <div className='flex items-center gap-4 pl-4'>
        ...
        <div className='text-right'>
          <div className={`text-sm font-semibold ${colors.text}`}>{completedDate}</div>
          {domain && (
            <div className='text-xs font-medium text-gray-500 dark:text-gray-400'>{domain}</div>
          )}
        </div>

        {/* Navigation arrow */}
        {href && (
          <a href={href} className={`${colors.link} transition`} title={linkText}>
